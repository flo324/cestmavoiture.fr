...
// Ta clé API de la capture f3c51b6c
...
interface ReparationFolder {
  id: string; titre: string; date: string; garage: string; adresse: string; km: string; prixTTC: string; details: string; imageUri: string;
}
...
export default function DocsScreen() {
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}><Text style={styles.headerTitle}>Mes réparations et entretiens</Text></View>
      <FlatList
        data={reparations}
        renderItem={({item}) => (
          <TouchableOpacity style={styles.folderCard} onPress={() => setSelectedRep(item)}>
            ...
            <View style={{marginLeft: 15, flex: 1}}>
              ...
              <Text style={{color: '#3498db', fontSize: 11}}>{item.date} • {item.km} km</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  folderCard: { backgroundColor: '#2c3e50', padding: 20, borderRadius: 20, marginBottom: 12, flexDirection: 'row', alignItems: 'center' },
  ...
  cropFrame: { width: width * 0.85, height: height * 0.55, borderWidth: 2, borderColor: '#fff', borderStyle: 'dashed', borderRadius: 10 },
