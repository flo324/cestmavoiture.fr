...
export function GarageConnectLogo({
  ...
  const cRotate = useSharedValue(0);
  ...
  useEffect(() => {
    ...
    // Départ : écartées et « penchées » comme prêtes à rouler
    gRotate.value = -34;
    cRotate.value = 34;
    gTx.value = -18;
    cTx.value = 18;

    // Roulement vers le centre, micro-choc, ressort jusqu’au calme
    gRotate.value = withSequence(
  ...
  const gStyle = useAnimatedStyle(() => ({
    transform: [{ translateX: gTx.value }, { rotate: `${gRotate.value}deg` }],
  }));

  const cStyle = useAnimatedStyle(() => ({
    transform: [{ translateX: cTx.value }, { rotate: `${cRotate.value}deg` }],
  }));
  ...
  return (
    <View style={styles.wrap}>
      <View style={[styles.row, { gap: d.gap }]}>
        {animated ? (
          <>
            <Animated.View style={[gStyle, outerAnimated]}>
              <LinearGradient
            ...
            <Animated.View style={[cStyle, outerAnimated]}>
              <LinearGradient
